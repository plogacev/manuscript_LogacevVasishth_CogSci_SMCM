...
library(lme4)
m1<-lmer(RT ~ (ambig+att)*(depth + shallow) + ( (ambig+att)*(depth + shallow) + 1||subj)+
          (1|item),crit)
summary(m1)
...
maov1 <- aov(RT ~ attachment*qtype + Error(subj/(attachment*qtype) ), crit)
summary(maov1)

maov2 <- aov(log(RT) ~ attachment*qtype + Error(subj/(attachment*qtype) ), crit)
summary(maov2)

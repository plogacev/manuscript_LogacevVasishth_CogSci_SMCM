...
m_nested<-lmer(RT~RCambig+RCatt+ Sambig+Satt+Oambig+Oatt+
                 ...
                 (1+RCambig+RCatt||item),crit)
summary(m_nested)

## ANOVA test on the overall effect of ambiguity in Superficial:
m_nestedNULL<-lmer(RT~RCambig+RCatt+Oambig+Oatt+
                 RC+NonRC+(1+RCambig+RCatt||subj)+
                   (1+RCambig+RCatt||item),crit)
...
On the log ms scale, we can find the expected effect of ambiguity in superficial questions:

<<>>=
m_nestedlog<-lmer(log(RT)~RCambig+RCatt+ Sambig+Satt+Oambig+Oatt+
                 RC+NonRC+(1+RCambig||subj)+(1+RCambig||item),crit)
summary(m_nestedlog)
@

The way Swets et al analyzed the data was by subsetting. So, if we subset the data to superficial question types only and then look at the effect of ambig and att, we get a marginal effect of ambiguity, as claimed in the paper. Actually, the effect is strong, if we analyze the data on log ms scale:

<<>>=
superficial<-subset(crit,qtype="superficial")
m3<-lmer(RT~Sambig+Satt+(1|subj)+(1|item),crit)
summary(m3)
m3log<-lmer(log(RT)~Sambig+Satt+(1|subj)+(1|item),crit)
summary(m3log)
@

Swets et al actually did an ANOVA to get at the crucial higher order interaction. So let's try that:

<<>>=
bysubjcrit<-aggregate(RT~subj+attachment + 
                        qtype,mean,data=crit)
library(rstatix)
res_anova<-anova_test(data = bysubjcrit, 
           dv = RT, 
           wid = subj,
           between = qtype, 
           within = attachment
...
byitemcrit<-aggregate(RT~item+attachment + 
                        qtype,mean,data=crit)

res_anova_item<-anova_test(data = byitemcrit, 
           dv = RT, 
           wid = item,
           #between = qtype, 
           within = c(qtype,attachment)
  )
get_anova_table(res_anova_item)
@

Next, we look at the superficial data (subsetted). This is an incorrect analysis because it hides the by items (in by-subject anova) or by subjects (in by-items anova) variability. We can reproduce the claim in the paper, although, as mentioned above, this analysis is incorrect:
...
byitemsup<-aggregate(RT~item+attachment,
                     mean,
                     data=superficial)

res_anova_itemsup<-anova_test(data = byitemsup, 
           dv = RT, 
           wid = item,
           #between = qtype, 
           within = attachment
  )
get_anova_table(res_anova_itemsup)

@

What happens with log RTs? Nothing much changes:

<<>>=
superficial$logrt<-log(superficial$RT)
bysubjsup<-aggregate(logrt~subj+attachment,mean,
                     data=superficial)
res_anovasuplog<-anova_test(data = bysubjsup, 
           dv = logrt, 
           wid = subj,
           within = attachment
  )
get_anova_table(res_anovasuplog)

byitemsuplog<-aggregate(logrt~item+attachment,
                     mean,
                     data=superficial)

res_anova_itemsuplog<-anova_test(data = byitemsuplog, 
           dv = logrt, 
           wid = item,
           #between = qtype, 
           within = attachment
  )
get_anova_table(res_anova_itemsuplog)
...
crit$attachment <- as.factor(crit$attachment) 
crit$qtype <- as.factor(crit$qtype) 

# check if we even have the right data
crit %>% group_by(attachment, qtype) %>%
  dplyr::summarise(M = mean(RT), SE = se(RT)) %>%
  ggplot(aes(attachment, M, color = attachment)) + facet_wrap(~qtype) + 
  geom_point() + geom_errorbar(aes(ymin=M-2*SE, ymax=M+2*SE)) +
...
# NOTE: Swets et al don't seem to have performed outlier removal: If I run the commented-out code below the average of the 
#       N1 attachment/RC questions condition drops to 1400, which is significantly smaller than in the paper.
# # drop all RTs > 5 sec (15 observations)
# nrow(subset(crit, RT > 5000))
# crit %<>% subset(RT <= 5000)

# let's do an ANOVA 
contrasts(crit$attachment) <- contr.sum(3)
contrasts(crit$qtype) <- contr.sum(3)

crit$subj %<>% as.character() %>% as.factor()
crit$item %<>% as.character() %>% as.factor()

#unique(crit$subj)

# Degrees of freedom match, but F values don't. They are in the ballpark, but still quite different
maov1 <- aov(RT ~ attachment*qtype + Error(subj/(attachment) ), crit)
summary(maov1)

# I think that this should be the random effects structure, but the DFs are off, which
# makes me think that they used another random effects structure.
# Needless to say, the F values don't match at all. 
maov2 <- aov(RT ~ attachment*qtype + Error(item/(attachment*qtype) ), crit)
summary(maov2)
